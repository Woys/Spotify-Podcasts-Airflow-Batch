...
def load_s3_file(s3_key: str, s3_bucket: str, tmp_dir: str, file_name: str):
    ...
    try:
        # Log the full S3 path being checked
...
def spotify_api_load(tmp_dir, chart_file, file_name):
    ...
    result_df = spotify_api.get_charts_eps_file(chart_file=chart_file)
    file_path = os.path.join(tmp_dir, file_name)
...
@dag(
    ...
    params = {
        "start_date": Param("2024-09-27T00:00:00+00:00",type="string",format="date-time"),
        "end_date": Param("2024-09-27T23:59:59+00:00",type="string",format="date-time"),
        "max_active_runs": 1,
        "retries": 2, 
        "retry_delay": duration(minutes=1),
        "catchup": False
